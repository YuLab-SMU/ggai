...
#' @param draw If `TRUE`, draw the current ggplot before returning it.
#'
#' @export
plot.ggai_session <- function(x, ..., draw = TRUE) {
  plot <- session_current_plot(x)
  if (isTRUE(draw)) {
    print(plot, ...)
  }
  invisible(plot)
...
#' @export
print.ggai_session <- function(x, ...) {
  state <- ggai_session_state(x)
  plot <- session_current_plot(x)
  layer_count <- length(plot$layers %||% list())
  artifact_count <- length(session_artifact_log(x))
  active_instruction <- state$active_instruction %||% ""

  cat("<ggai_session>\n")
  cat("Current plot: ggplot with ", layer_count, " layer", if (layer_count == 1L) "" else "s", "\n", sep = "")
  cat("Turns: ", x$history_index %||% 0L, "\n", sep = "")
  if (nzchar(active_instruction)) {
    wrapped <- strwrap(active_instruction, width = 72, prefix = "Instruction: ", exdent = 13)
    cat(paste(wrapped, collapse = "\n"), "\n", sep = "")
  }
  if (artifact_count) {
    cat("Artifacts: ", artifact_count, "\n", sep = "")
  }
  if (!length(x$history %||% list())) {
    cat("Model calls: none yet; the initial chart is local ggplot inference.\n")
  }
  cat("Use plot(x), gg_edit(x, ...), spec_history(x), or session_context(x).\n")
  invisible(x)
}
