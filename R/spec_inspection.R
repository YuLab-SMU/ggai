...
render_spec_compiled <- function(compiled, plot = NULL, data = NULL) {
  ...
  base_plot <- if (inherits(plot, "ggplot")) plot_base_plot(plot) else ggplot2::ggplot(data = data %||% NULL)
  layers <- compile_layer_spec_to_layers(compiled$spec, data = data %||% base_plot$data)
  out <- base_plot
  for (layer in layers) {
    out <- out + layer
  }
